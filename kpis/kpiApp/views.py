...
def addRepresentante_view(request):
	info = "Inicializando" 
	if request.method == "POST":
		form = addRepresentanteForm(request.POST)
		if form.is_valid():
			add = form.save(commit=False) 
			add.status = True
			add.save() #Guarda la informacion
		...
		form = addRepresentanteForm()
	ctx = {'form':form, 'informacion':info}
	return render_to_response('addRepresentante.html',ctx,context_instance=RequestContext(request))
